...
class ModelManager:
    ...
    def load_remove_modules(self, file_path="", state_dict=None, is_final=True):
        """
        Load only the following modules from the given file(s):
        - condition_adapter
        - task_fg_tokenizer

        LoRA-related loading and alpha scaling have been completely removed.
        The collected weights are stored in self.state_dict_new_module.
        """

        def _collect_from_state(sd):
            matched = 0
            # import pdb
            # pdb.set_trace()
            for k, v in sd.items():
                if "insert_condition_adapter" in k or "task_fg_tokenizer" in k or "remove_condition_adapter" in k or "CrossAttHeadProb" in k:
                    self.state_dict_new_module[k] = v
                    matched += 1
            return matched

        if isinstance(file_path, list):
            total_matched = 0
            for idx, path in enumerate(file_path, start=1):
                is_final = (idx == len(file_path))
                print(f"[Modules] Loading condition/task tokenizer from: {path}")
                sd = state_dict if state_dict else load_state_dict(path)
                total_matched += _collect_from_state(sd)

            if total_matched == 0:
                print("[Modules] No condition_adapter/task_fg_tokenizer keys found in provided files.")
            else:
                print(f"[Modules] Collected {total_matched} module tensors.")
        else:
            print(f"[Modules] Loading condition/task tokenizer from: {file_path}")
            if not state_dict:
                state_dict = load_state_dict(file_path)

            matched = _collect_from_state(state_dict)
            if matched == 0:
                print(f"[Modules] No condition_adapter/task_fg_tokenizer keys found in: {file_path}")
            else:
                print(f"[Modules] Collected {matched} module tensors from: {file_path}")
    ...
    def load_lora_v2(self, file_path="", state_dict={}, lora_alpha=1.0, is_final = True):
        if isinstance(file_path, list):
             ...
             for file_path_ in file_path:
                ...
                # self.load_lora_v2(file_path_, state_dict=None, lora_alpha=lora_alpha, is_final=is_final)
                ii += 1
        else:
            ...
            for key in state_dict.keys():
                if "remove_condition_adapter" in key or "task_fg_tokenizer" in key or "insert_condition_adapter" in key or "CrossAttHeadProb" in key:
                    ...
                    for model_name, model, model_path in zip(self.model_name, self.model, self.model_path):
                        for lora in get_lora_loaders():
                            ...
                            if match_results is not None:
                                ...
                                break
            if not is_loaded:
                print(f"    Failed to load LoRA from {file_path}.")
